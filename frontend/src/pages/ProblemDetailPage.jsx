...
export default function ProblemDetailPage({ username }) {
  ...
  const [hasLeetCodeDesc, setHasLeetCodeDesc] = useState(false);
  ...
  useEffect(() => {
    fetch(`/api/problems/${id}`)
      ...
      .then(data => {
        ...
        setLoading(false);
        // Auto-fetch full LeetCode description
        const slug = data.id.startsWith('prob-') ? data.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') : data.id;
        const q = `query questionContent($titleSlug: String!) { question(titleSlug: $titleSlug) { content title difficulty exampleTestcases topicTags { name slug } } }`;
        fetch('/api/leetcode-graphql', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ query: q, variables: { titleSlug: slug } })
        }).then(r => r.json()).then(res => {
          const c = res?.data?.question?.content;
          if (!c) return;
          const html = c
          ...
          setProblem(prev => prev ? { ...prev, description: html } : prev);
          setHasLeetCodeDesc(true);
        }).catch(() => {});
        if (data.category) {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <div>
          ...
          <div className="card" style={{ padding: 24, marginBottom: 16, fontSize: 14, lineHeight: 1.7 }}>
            ...
            {!hasLeetCodeDesc && <div style={{ fontSize: 13, fontWeight: 600, margin: '16px 0 8px' }}>Examples</div>}
            {!hasLeetCodeDesc && Array.isArray(problem.examples) && problem.examples.map((ex, i) => (
              <div key={i} style={{
