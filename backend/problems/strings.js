export default [
  ...
  },
  {
    ...
    approach: "The problem is to find the longest string that is a prefix of every string in a given array, meaning the result must appear at the start of each input string. A brute-force approach might generate all possible prefixes of the shortest string and test each against all other strings, which is inefficient. The optimal approach uses vertical scanning with the first string as the initial reference prefix. Start by setting the prefix to the first string in the array. Then iterate through each of the remaining strings one by one. For each comparison, examine characters at the same position between the current prefix and the next string, stopping when either a mismatch occurs or the end of either string is reached. Determine the common prefix length from this scan and truncate the prefix to that length. Continue this process for all strings in the array. After processing every string, the remaining prefix is the longest common prefix shared by all strings. For input ['flower', 'flow', 'flight'], start with prefix 'flower'. Compare with 'flow': scanning finds 'flow' as the common prefix. Compare 'flow' with 'flight': scanning character by character finds 'fl' as the common prefix, which is the final answer. For ['dog', 'racecar', 'car'], comparing 'dog' with 'racecar' immediately gives empty string as the common prefix since the first characters 'd' and 'r' differ. Edge cases include an empty input array returning an empty string, a single-element array where the string itself is the common prefix, and cases where the first string is the shortest making it the prefix bound. Time complexity is O(n * m) where n is the number of strings and m is the length of the shortest string, but early termination on mismatch makes it faster in practice. Space complexity is O(1) beyond the prefix string.\n\nDiagram:\n```\nlongest-common-prefix:\n  strs = [\"flower\", \"flow\", \"flight\"]\n\n  prefix = \"flower\"\n  Compare with \"flow\":\n    f==f ✓  l==l ✓  o==o ✓  w==w ✓\n    |\"flow\"| = 4 → truncate to \"flow\"\n\n  prefix = \"flow\"\n  Compare with \"flight\":\n    f==f ✓  l==l ✓  o!=i ✗ → stop\n    Common = 2 → truncate to \"fl\"\n\n  Result: \"fl\"\n```",
  ...
  },
  {
    ...
    test_cases: [{"input":"()[]{}","expected":"Yes"},{"input":"(]","expected":"No"}],
    solution_template: "#include <iostream>\n#include <stack>\nusing namespace std;\n\nint main() {\n  string s;\n  cin >> s;\n\n  stack<char> st;\n\n  cout << (valid ? \"Yes\" : \"No\") << endl;\n  return 0;\n}",
    approach: "The problem is to validate whether a string consisting of parentheses '()', curly braces '{}', and square brackets '[]' is correctly balanced and nested. A brute-force approach might repeatedly replace matching bracket pairs until the string is empty or no replacements are possible, which runs in O(n^2) time and is cumbersome. The optimal approach uses a stack data structure, which naturally models the last-in-first-out nature of bracket nesting. Initialize an empty stack, then iterate through each character of the string. If the character is any opening bracket, push it onto the stack. If it is a closing bracket, first check whether the stack is empty, which would indicate a closing bracket with no matching opening bracket, making the string invalid. If the stack is not empty, pop the top character from the stack and check if it forms a matching pair with the current closing bracket. Specifically, a closing ')' must pair with '(' at the top, a closing '}' must pair with '{', and a closing ']' must pair with '['. If the pair does not match, the string is invalid. After processing all characters, the string is valid only if the stack is completely empty, meaning every opening bracket had a corresponding closing bracket in the correct order. For '()[]{}', the stack pushes '(' then pops it on ')', pushes '[' pops on ']', pushes '{' pops on '}', ending empty. For '(]', the stack pushes '(' then encounters ']' but the top is '(' which does not match, so it fails. Edge cases include a single opening bracket like '(' where the stack remains non-empty at the end, and a closing bracket alone like ']' where the stack is empty at the time of checking. Time complexity is O(n) for a single pass through the string, and space is O(n) in the worst case when all characters are opening brackets awaiting closure.\n\nDiagram:\n```\nvalid-parentheses:\n  s = \"()[]{}\"\n\n  Step 1: c='(' → push → Stack: ['(']\n  Step 2: c=')' → pop → top='(' matches ')' → Stack: []\n  Step 3: c='[' → push → Stack: ['[']\n  Step 4: c=']' → pop → top='[' matches ']' → Stack: []\n  Step 5: c='{' → push → Stack: ['{']\n  Step 6: c='}' → pop → top='{' matches '}' → Stack: []\n\n  Stack empty → valid ✓\n```",
    complexity: {"time":"O(n)","space":"O(n)"},
    techniques: [],
    sheet: "Striver A2Z",
  ...
  },
  {
    ...
    test_cases: [{"input":"babad","expected":"bab"},{"input":"cbbd","expected":"bb"}],
    solution_template: "#include <iostream>\nusing namespace std;\n\nint main() {\n  string s;\n  cin >> s;\n\n  // expand around center\n\n  cout << result << endl;\n  return 0;\n}",
    approach: "The task is to find the longest contiguous substring that reads the same forwards and backwards. A brute-force solution would enumerate every possible substring and test whether it is a palindrome, which costs O(n^3) time since there are O(n^2) substrings and each palindrome check takes O(n) time. The optimal expand-around-center approach exploits the fact that every palindrome has a center and mirrors outward from it. There are 2n-1 possible centers in a string of length n: n odd-length centers at each character position, and n-1 even-length centers between consecutive characters. For each center, expand outward by moving a left pointer leftward and a right pointer rightward as long as the characters at both pointers are equal and within bounds. Track the current palindrome length and update a global maximum length and the corresponding substring boundaries whenever a longer palindrome is found. For odd-length palindromes, set left equal to right equal to the current index i, then expand. For even-length palindromes, set left equal to i and right equal to i+1, then expand. For 'babad', starting at center i=0, odd expansion yields 'b' of length 1. At i=1, odd expansion yields 'bab' of length 3, and even expansion between i=1 and i=2 yields nothing. At i=2, odd expansion yields 'aba' also length 3. The maximum length found is 3, and either 'bab' or 'aba' is a valid answer. For 'cbbd', the even expansion at i=1 and i=2 yields 'bb' of length 2, which is the longest. Edge cases include a single character string where the substring is the character itself, and strings with no palindrome longer than 1 where any single character is correct. Time complexity is O(n^2) because each of the 2n-1 centers can expand up to O(n) in the worst case, and space complexity is O(1) as only pointer variables and length trackers are needed.\n\nDiagram:\n```\nlongest-palindromic-substr:\n  s = \"babad\"\n\n  Odd centers:\n  i=0: [b]        → \"b\" (len=1)\n  i=1: [b a b]    → \"bab\" (len=3)  ← max\n  i=2: [a b a]    → \"aba\" (len=3)\n  i=3: [a d]      → no match\n  i=4: [d]        → \"d\" (len=1)\n\n  Even centers:\n  i=0,1: \"ba\" → no\n  i=1,2: \"ab\" → no\n  i=2,3: \"ba\" → no\n  i=3,4: \"ad\" → no\n\n  Result: \"bab\"\n```",
    complexity: {"time":"O(n²)","space":"O(1)"},
    techniques: ["two-pointers"],
    sheet: "Striver A2Z",
  ...
  },
  {
    ...
    test_cases: [{"input":"sadbutsad sad","expected":"0"},{"input":"leetcode leeto","expected":"-1"}],
    solution_template: "#include <iostream>\nusing namespace std;\n\nint main() {\n  string haystack, needle;\n  cin >> haystack >> needle;\n\n  // sliding window or KMP\n\n  cout << index << endl;\n  return 0;\n}",
    approach: "The strStr problem asks for the index of the first occurrence of a pattern (needle) within a text (haystack), returning -1 if the needle is not found. The brute-force sliding window approach slides over each position of the haystack and compares the needle character by character, which requires O(n * m) time in the worst case where n is the haystack length and m is the needle length. The KMP (Knuth-Morris-Pratt) algorithm improves this to O(n + m) by preprocessing the needle to compute a Longest Prefix Suffix (LPS) array. The LPS array stores, for each prefix of the needle, the length of the longest proper prefix that is also a suffix of that prefix. To build the LPS array, initialize a pointer j to 0 and iterate i from 1 to m-1. When needle[i] matches needle[j], increment j and set LPS[i] to j, then increment i. When they do not match and j is not zero, fall back j to LPS[j-1] without advancing i. When j is zero and characters differ, set LPS[i] to 0 and advance i. Once LPS is built, perform the matching phase using two pointers: i for the haystack and j for the needle. While i is less than n, compare haystack[i] to needle[j]. If they match, increment both i and j. If j reaches m, a match has been found at index i - m and we return that index. On a mismatch, if j is not zero, set j to LPS[j-1] without advancing i; otherwise advance i. For haystack 'sadbutsad' and needle 'sad', LPS of 'sad' is [0,0,0] and matching finds the pattern directly at index 0. For 'leetcode' and 'leeto', the LPS array reveals no full match. Edge cases include an empty needle in which the answer by convention is 0, and a needle longer than the haystack which immediately returns -1. Time complexity is O(n + m) for building the LPS array and performing the match, and space is O(m) for storing the LPS array.\n\nDiagram:\n```\nstrstr:\n  haystack = \"sadbutsad\"\n  needle   = \"sad\"\n\n  i=0: s a d\n       ↑ ↑ ↑\n       s a d  → all match! return 0\n\n  LPS for \"sad\": [0,0,0]\n\n  Result: 0\n```",
  ...
  },
  {
    ...
    test_cases: [{"input":"MCMXCIV","expected":"1994"},{"input":"LVIII","expected":"58"}],
    solution_template: "#include <iostream>\nusing namespace std;\n\nint romanValue(char c) {\n  switch(c) {\n    case 'I': return 1;\n    case 'V': return 5;\n    case 'X': return 10;\n    case 'L': return 50;\n    case 'C': return 100;\n    case 'D': return 500;\n    case 'M': return 1000;\n    default: return 0;\n  }\n}\n\nint main() {\n  string s;\n  cin >> s;\n\n  // add normally, subtract if smaller before larger\n\n  cout << result << endl;\n  return 0;\n}",
    approach: "Roman numerals use seven symbols with fixed integer values and are typically written from largest to smallest from left to right, but when a smaller value precedes a larger value it indicates a subtractive combination such as IV for 4 or IX for 9. A brute-force approach would enumerate all subtractive cases individually using conditional checks, which is tedious and does not generalize well. The optimal approach processes the string left to right with a single look-ahead comparison. First, build a lookup table mapping each Roman character to its integer value: I=1, V=5, X=10, L=50, C=100, D=500, M=1000. Initialize the result to 0 and iterate over each character at position i. Compare the value of the current character with the value of the next character at position i+1 if it exists. If the current value is less than the next value, the current character is part of a subtractive pair, so subtract its value from the result. Otherwise, add its value to the result. This single rule elegantly handles both normal additive numerals and subtractive combinations without special-case logic. For 'MCMXCIV', start with M=1000 which is not less than C=100, so add 1000. Next C=100 is less than M=1000, so subtract 100, giving 900. Then M=1000 is not less than X=10, add 1000 to get 1900. X=10 is less than C=100, subtract 10 to get 1890. C=100 is not less than I=1, add 100 to get 1990. I=1 is less than V=5, subtract 1 to get 1989. Finally V=5 has no next character, add 5 to get 1994. Edge cases include single-character input where there is no next character so the value is simply added, and the input being guaranteed by constraints to be a valid Roman numeral in the range 1 to 3999. Time complexity is O(n) for a single linear pass through the string, and space complexity is O(1) for the fixed-size lookup table.\n\nDiagram:\n```\nroman-to-int:\n  s = \"MCMXCIV\"\n  val: I=1 V=5 X=10 L=50 C=100 D=500 M=1000\n\n  M=1000 < C=100? No → +1000 = 1000\n  C=100 < M=1000? Yes → -100  = 900\n  M=1000 < X=10? No  → +1000 = 1900\n  X=10 < C=100? Yes  → -10   = 1890\n  C=100 < I=1? No    → +100  = 1990\n  I=1 < V=5? Yes     → -1    = 1989\n  V=5 (last)         → +5    = 1994\n\n  Result: 1994\n```",
    complexity: {"time":"O(n)","space":"O(1)"},
    techniques: [],
    sheet: "Striver A2Z",
  ...
  },
  {
    ...
    constraints: "1 <= |s| <= 10^4",
    examples: [{"input":"aabbbcccc","output":"a2b3c4"}],
    test_cases: [{"input":"aabbbcccc","expected":"a2b3c4"},{"input":"abc","expected":"a1b1c1"}],
    solution_template: "#include <iostream>\nusing namespace std;\n\nint main() {\n  string s;\n  cin >> s;\n\n  // count consecutive chars\n\n  cout << encoded << endl;\n  return 0;\n}",
    approach: "The problem is to perform run-length encoding on a string, compressing consecutive repeated characters into a single character followed by the count of its repetitions. A brute-force approach using nested loops would be unnecessarily complex for this straightforward linear task. The optimal approach uses a single pass with a counter to track consecutive repetitions. Initialize an empty result string and a count variable set to 1. Iterate through the string starting from index 1 to the end. At each step, compare the current character with the previous character. If they are equal, increment the count. If they are different, this marks the end of a run of identical characters: append the previous character and its count as a string to the result, then reset the count to 1 for the new character. After the loop ends, one more append is needed for the last run of characters. For 'aabbbcccc', start with count=1. At i=1, 'a' equals the previous 'a', so count becomes 2. At i=2, 'b' differs from 'a', so append 'a2' and reset count to 1. At i=3, 'b' equals previous 'b', count becomes 2. At i=4, 'b' equals previous 'b', count becomes 3. At i=5, 'c' differs from 'b', so append 'b3' and reset count to 1. For i=6,7,8, 'c' repeats, count reaches 4. After the loop, append 'c4'. The result is 'a2b3c4'. For 'abc' where no consecutive repeats occur, the result is 'a1b1c1' since each character appears exactly once. Edge cases include an empty string where the result is also an empty string, and a single character string where the result is the character followed by 1. Time complexity is O(n) for a single linear pass through the string, and space complexity is O(n) for the result string which in the worst case where no compression occurs is twice the size of the input.\n\nDiagram:\n```\nstring-compression:\n  s = \"aabbbcccc\"\n\n  i=0: 'a' → cnt=1\n  i=1: 'a'==prev → cnt=2\n  i=2: 'b'!=prev → append \"a2\", cnt=1\n  i=3: 'b'==prev → cnt=2\n  i=4: 'b'==prev → cnt=3\n  i=5: 'c'!=prev → append \"b3\", cnt=1\n  i=6: 'c'==prev → cnt=2\n  i=7: 'c'==prev → cnt=3\n  i=8: 'c'==prev → cnt=4\n  end: append \"c4\"\n\n  Result: \"a2b3c4\"\n```",
    complexity: {"time":"O(n)","space":"O(n)"},
    techniques: ["two-pointers"],
    sheet: "Love Babbar 450",
  ...
  },
  {
    ...
    approach: "The goal is to group together strings that are anagrams of each other, meaning they contain exactly the same characters but possibly in different orders. A brute-force approach would compare every pair of strings to check if they are anagrams, which requires O(n^2 * k) time where k is the average string length. The optimal approach uses a hash map where the key is a canonical representation of each string. Since two strings are anagrams if and only if their sorted character sequences are identical, the most straightforward canonical form is the string itself sorted alphabetically. Iterate through every input string, create a copy, sort its characters, and use this sorted string as the key in a hash map. The value associated with each key is a list of all original strings that generated that sorted key. After processing all input strings, iterate through the hash map and output each list as a separate group of anagrams. For input ['ate', 'eat', 'tea', 'tan', 'nat', 'bat'], sorting 'ate' yields 'aet', sorting 'eat' also yields 'aet', and sorting 'tea' also yields 'aet', so these three strings are grouped together under the key 'aet'. Sorting 'tan' and 'nat' both yield 'ant', forming a second group. Sorting 'bat' yields 'abt', which is unique, forming a singleton group. Edge cases include empty strings which all sort to the empty string and are grouped together, and strings of different lengths which automatically produce different sorted keys. Time complexity is O(n * k log k) where n is the number of strings and k is the maximum string length, with the sorting step being the dominant factor. Space complexity is O(n * k) to store all strings distributed across the hash map groups.\n\nDiagram:\n```\ngroup-anagrams:\n  Input: [\"ate\",\"eat\",\"tea\",\"tan\",\"nat\",\"bat\"]\n\n  Sorted signatures:\n  ate → aet\n  eat → aet\n  tea → aet\n  tan → ant\n  nat → ant\n  bat → abt\n\n  HashMap:\n  aet → [\"ate\",\"eat\",\"tea\"]\n  ant → [\"tan\",\"nat\"]\n  abt → [\"bat\"]\n\n  Output:\n  ate eat tea\n  tan nat\n  bat\n```",
  ...
  },
  {
    ...
    constraints: "1 <= |s| <= 1000",
    examples: [{"input":"aaa","output":"6","explanation":"a,a,a,aa,aa,aaa"}],
    test_cases: [{"input":"aaa","expected":"6"},{"input":"abc","expected":"3"}],
    solution_template: "#include <iostream>\nusing namespace std;\n\nint main() {\n  string s;\n  cin >> s;\n\n  // expand around center for each position\n\n  cout << count << endl;\n  return 0;\n}",
    approach: "The problem asks to count how many substrings of a given string are palindromes, where substrings are contiguous sequences of characters. A brute-force method would generate every possible substring and test each one individually for being a palindrome, leading to O(n^3) time because there are O(n^2) substrings and each check takes O(n) time. The optimal expand-around-center approach counts all palindromic substrings in O(n^2) time by leveraging the symmetric nature of palindromes. A palindrome mirrors around its center, and there are 2n-1 possible centers in a string of length n: n odd-length centers at each individual character, and n-1 even-length centers between consecutive characters. For each center, expand outward with a left pointer moving left and a right pointer moving right as long as the characters match and both pointers stay within the string boundaries. Each successful expansion corresponds to exactly one palindromic substring, so increment a counter for each match found. For odd-length centers, start with left and right both at the same index i and expand outward. For even-length centers, start with left at i and right at i+1 and expand outward. For 'aaa', odd centers at i=0 expand to 'a'(+1), then left=-1 stops; at i=1 expand to 'a'(+1), then 'aaa'(+1) with left=0,right=2, then left=-1 stops, total 2 from this center; at i=2 expand to 'a'(+1). Odd total is 4. Even centers: between i=0 and i=1 expand to 'aa'(+1) then stop; between i=1 and i=2 expand to 'aa'(+1) then stop. Even total is 2, giving a grand total of 6 which matches the expected output. For 'abc', each odd center yields exactly one single-character palindrome, and no even expansions ever match, so the total is 3. Edge cases include a single character string returning 1 and strings with all unique characters returning n where n is the string length. Time complexity is O(n^2) as each center may expand O(n) times, and space complexity is O(1) using only integer variables.\n\nDiagram:\n```\ncount-palindromic-substr:\n  s = \"aaa\"\n\n  Odd centers:\n  i=0: \"a\" (+1)\n  i=1: \"a\" (+1), \"aaa\" (+1)\n  i=2: \"a\" (+1)\n  Odd total: 4\n\n  Even centers:\n  i=0,1: \"aa\" (+1)\n  i=1,2: \"aa\" (+1)\n  Even total: 2\n\n  Grand total: 6\n```",
    complexity: {"time":"O(n²)","space":"O(1)"},
    techniques: ["two-pointers"],
    sheet: "Love Babbar 450",
  ...
  },
  {
    ...
    constraints: "1 <= |s| <= 10^5",
    examples: [{"input":"aacecaaa","output":"1","explanation":"Add \"a\" at front -> \"aaacecaaa\""}],
    test_cases: [{"input":"aacecaaa","expected":"1"}],
    solution_template: "#include <iostream>\n#include <algorithm>\nusing namespace std;\n\nint main() {\n  string s;\n  cin >> s;\n\n  // use LPS (KMP) on s + '#' + reverse(s)\n\n  cout << add << endl;\n  return 0;\n}",
    approach: "The problem asks for the minimum number of characters that must be added at the front of a string to make the entire string a palindrome. A brute-force approach would try appending prefixes of increasing length and checking whether the resulting string is a palindrome, which takes O(n^2) time. The optimal approach uses the LPS (Longest Prefix Suffix) array from the KMP algorithm to find the longest palindromic prefix of the original string in O(n) time. The critical insight is that if we already have a palindromic prefix of length L starting at position 0, we only need to add the reverse of the remaining n-L characters at the front. To find the longest palindromic prefix efficiently, construct a new combined string T by concatenating the original string s, a separator character '#' that does not appear in s, and the reverse of s. The separator prevents the LPS computation from matching across the boundary between s and its reverse in unintended ways. Compute the LPS array for T using the standard KMP preprocessing: iterate through T with index i from 1 to |T|-1, and maintain a pointer j for the length of the current longest proper prefix that is also a suffix. The last value of the LPS array, LPS[|T|-1], represents the length of the longest prefix of T that is also a suffix of T, which corresponds exactly to the longest palindrome that starts at the beginning of s. The minimum number of characters to add at the front is therefore n minus LPS[last]. For 'aacecaaa', T is 'aacecaaa#aaasecaa', the LPS at the last position is 7, meaning the longest palindromic prefix is 'aacecaa' of length 7, so only 1 character 'a' needs to be added. For 'abcd', T is 'abcd#dcba', LPS at the last position is 1 (just the character 'a'), so 3 characters need to be added. Edge cases include a string that is already a palindrome where LPS[last] equals n and the result is 0, and a single character where the result is trivially 0. Time complexity is O(n) for building the LPS array, and space complexity is O(n) for the combined string and the LPS array.\n\nDiagram:\n```\nmin-chars-palindrome:\n  s = \"aacecaaa\"\n\n  T = s + '#' + reverse(s)\n  T = \"aacecaaa#aaasecaa\"\n  LPS last value = 7\n\n  Longest palindromic prefix = first 7 chars = \"aacecaa\"\n  Need to add = |s| - 7 = 1 character\n\n  Add \"a\" at front → \"aaacecaaa\" ✓\n```",
    complexity: {"time":"O(n)","space":"O(n)"},
    techniques: ["two-pointers"],
    sheet: "Love Babbar 450",
  ...
  },
  {
    ...
    examples: [{"input":"aa a*","output":"Yes"}],
    test_cases: [{"input":"aa a*","expected":"Yes"},{"input":"cb ?a","expected":"No"}],
    solution_template: "#include <iostream>\nusing namespace std;\n\nint main() {\n  string s, p;\n  cin >> s >> p;\n\n  // 2D DP\n\n  cout << (match ? \"Yes\" : \"No\") << endl;\n  return 0;\n}",
    approach: "The problem is to match a string s against a pattern p that contains two special wildcard characters: '?' which matches any single character, and '*' which matches any sequence of characters including the empty sequence. A brute-force recursive approach with backtracking at each '*' would branch exponentially, making it infeasible for strings up to length 2000. The optimal solution uses dynamic programming with a 2D boolean table dp of size (n+1) by (m+1), where dp[i][j] is true if the first i characters of s match the first j characters of p. Initialize dp[0][0] as true since two empty strings match. For the first row where i is 0, dp[0][j] is set to dp[0][j-1] if p[j-1] is '*', because a '*' can match the empty string. Then fill the table row by row: for each i from 1 to n and each j from 1 to m, if the pattern character p[j-1] is '*', then dp[i][j] is true if either dp[i-1][j] is true (the '*' matches the current character s[i-1] and stays in place for further characters) or dp[i][j-1] is true (the '*' matches the empty string and is consumed). If p[j-1] is '?' or s[i-1] equals p[j-1], then dp[i][j] is set to dp[i-1][j-1] since a single character match is required. Otherwise, dp[i][j] is false. After filling the entire table, the answer is dp[n][m]. For s='aa' and p='a*', dp[2][2] evaluates to true because '*' at position 2 matches the second 'a'. For s='cb' and p='?a', dp[2][2] is false because after '?' matches 'c', the expected 'a' does not match 'b'. Edge cases include a pattern that is just '*', which matches any string by matching the entire string via repeated application of the star-matches-character rule; and a pattern with leading '*' that can skip leading characters. Time complexity is O(n * m) for filling the 2D DP table, and space complexity is O(n * m) which can be optimized to O(m) using two rolling rows if needed.\n\nDiagram:\n```\nwildcard-matching:\n  s = \"aa\", p = \"a*\"\n\n  DP Table (T=true, F=false):\n       ∅    a    *\n    ┌────────────────\n  ∅ │ T    F    T\n  a │ F    T    T\n  a │ F    F    T\n\n  dp[2][2] = dp[1][2] || dp[2][1]\n           = T || F = T → match ✓\n```",
  ...
  },
  {
    ...
    category: "strings",
    difficulty: "medium",
    description: "Find the longest substring containing the same character after at most k replacements.",
    constraints: "1 <= |s| <= 10^5, 0 <= k <= 10^5",
    examples: [{"input":"ABAB 2","output":"4","explanation":"Replace both B's with A → AAAA length 4"}],
    test_cases: [{"input":"ABAB 2","expected":"4"},{"input":"AABABBA 1","expected":"4"}],
    solution_template: "#include <iostream>\nusing namespace std;\n\nint main() {\n  string s;\n  int k;\n  cin >> s >> k;\n\n  // sliding window + freq array\n\n  cout << maxLen << endl;\n  return 0;\n}",
    approach: "The problem asks for the longest substring that can be made uniform (all same character) after at most k character replacements. A brute-force approach checking every substring would be O(n^2). The optimal solution uses a sliding window with a frequency array tracking character counts within the current window. Expand the right pointer and increment the count for the new character. Track the maximum frequency count of any single character within the window. The key insight is that the number of replacements needed for the window equals (window length - max frequency). If this exceeds k, shrink the window by moving the left pointer right and decrementing the count of the character being removed. Throughout the process, track the maximum window length seen. For 'ABAB' with k=2, the window can grow to include all 4 characters because the max frequency is 2 (either A or B), and 4-2=2 replacements needed, which is exactly k.\n\nDiagram:\n```\nlongest-repeating-replace:\n  s = \"AABABBA\", k = 1\n\n  Step 1: [A]         l=0,r=0 maxFreq=1  need=0  len=1\n  Step 2: [A A]       l=0,r=1 maxFreq=2  need=0  len=2\n  Step 3: [A A B]     l=0,r=2 maxFreq=2  need=1  len=3\n  Step 4: [A A B A]   l=0,r=3 maxFreq=3  need=1  len=4  ← max\n  Step 5: A[A B A B]  l=1,r=4 maxFreq=2  need=2>k → shrink\n  Step 6: A A[B A B]  l=2,r=4 maxFreq=2  need=1  len=3\n\n  Max length = 4\n```",
    complexity: {"time":"O(n)","space":"O(1)"},
    techniques: ["sliding-window"],
    sheet: "Striver A2Z",
  ...
  },
  {
    ...
    examples: [{"input":"leetcode leet code","output":"Yes","explanation":"leetcode = leet + code"}],
    test_cases: [{"input":"leetcode leet code","expected":"Yes"},{"input":"catsandog cats dog sand and cat","expected":"No"}],
    solution_template: "#include <iostream>\n#include <unordered_set>\nusing namespace std;\n\nint main() {\n  string s;\n  int n;\n  cin >> s >> n;\n  string dict[n];\n  for(int i=0;i<n;i++) cin >> dict[i];\n\n  // DP or Trie + DP\n\n  cout << (possible ? \"Yes\" : \"No\") << endl;\n  return 0;\n}",
    approach: "The problem is to determine whether a string can be segmented into a sequence of words from a given dictionary. This is a classic DP problem where a Trie can accelerate word lookup. Build a Trie from all dictionary words. Then use a DP array dp[i] indicating whether the prefix s[0..i-1] can be segmented. dp[0] is true for the empty prefix. For each position i where dp[i] is true, traverse the Trie starting from the root and follow characters from position i onward, marking dp[j] as true whenever a complete word is found ending at position j. After processing all positions, dp[n] indicates whether the entire string can be segmented. For 'leetcode' with dictionary ['leet','code'], dp[0]=T, then matching 'leet' from index 0 gives dp[4]=T, then matching 'code' from index 4 gives dp[8]=T, so the answer is Yes.\n\nDiagram:\n```\nword-break:\n  s = \"leetcode\"\n  dict = [\"leet\", \"code\"]\n  Trie: root→l→e→e→t[*]→c→o→d→e[*]\n\n  DP:\n  dp[0] = T (empty prefix)\n  dp[4] = T (s[0-3]=\"leet\" found in Trie)\n  dp[8] = T (s[4-7]=\"code\" found in Trie)\n\n  Result: Yes\n```",
    complexity: {"time":"O(n^2)","space":"O(n + total_chars_in_dict)"},
    techniques: ["trie"],
    sheet: "Love Babbar 450",
  ...
  },
  {
    ...
    category: "strings",
    difficulty: "medium",
    description: "Check if a word exists in a 2D board of characters via adjacent cells.",
    constraints: "1 <= m,n <= 6, 1 <= |word| <= 15",
    examples: [{"input":"3 4\\nABCE\\nSFCS\\nADEE\\nABCCED","output":"Yes"}],
    test_cases: [{"input":"3 4\\nABCE\\nSFCS\\nADEE\\nABCCED","expected":"Yes"},{"input":"3 4\\nABCE\\nSFCS\\nADEE\\nABCB","expected":"No"}],
    solution_template: "#include <iostream>\n#include <vector>\nusing namespace std;\n\nint main() {\n  int m,n; cin>>m>>n;\n  vector<vector<char>> board(m,vector<char>(n));\n  for(int i=0;i<m;i++)for(int j=0;j<n;j++)cin>>board[i][j];\n  string word; cin>>word;\n\n  // backtracking DFS\n\n  cout << (found ? \"Yes\" : \"No\") << endl;\n  return 0;\n}",
    approach: "The problem asks whether a given word exists in a 2D character board, moving through adjacent (up/down/left/right) cells without revisiting any cell. This is solved via backtracking DFS from each cell that matches the first character. From each starting cell, recursively explore all four directions, tracking the current index in the word. Mark visited cells temporarily to avoid reuse. If the current character matches, continue searching the next character in all four adjacent cells. If the entire word is matched (index reaches word length), return true. If no path succeeds, backtrack by unmarking the cell and try other branches. For a board of size up to 6x6 and word length up to 15, the search space is manageable with pruning.\n\nDiagram:\n```\nword-search:\n  Board: A B C E\n         S F C S\n         A D E E\n  Word: \"ABCCED\"\n\n  Start at (0,0)='A' ✓\n  → (0,1)='B' ✓\n  → (0,2)='C' ✓\n  → (0,3)='E' ✗ back\n  → (1,2)='C' ✓\n  → (1,3)='S' ✗ → (2,2)='E' ✓\n  → (2,1)='D' ✓\n  Found: A→B→C→C→E→D → \"ABCCED\" ✓\n```",
  ...
  },
  {
    ...
    test_cases: [{"input":"3","expected":"((())) (()()) (())() ()(()) ()()()"}],
    solution_template: "#include <iostream>\n#include <vector>\nusing namespace std;\n\nint main() {\n  int n;\n  cin >> n;\n\n  // backtracking with open/close counts\n\n  return 0;\n}",
    approach: "The task is to generate all well-formed parentheses strings for n pairs. A well-formed string must have balanced opening and closing brackets at every prefix. Use backtracking with two counters: open (number of '(' placed) and close (number of ')' placed). Start with an empty string. At each step, if open is less than n, add '(' and recurse. If close is less than open, add ')' and recurse. When both open and close equal n, a valid combination is complete. For n=3, the recursion tree builds 5 valid combinations by always maintaining the invariant that close never exceeds open, and neither exceeds n.\n\nDiagram:\n```\ngenerate-parens:\n  n = 3\n\n  Recursion tree:\n  \"\" → \"(\" → \"((\" → \"(((\" → \"((()\" → \"((())\" → \"((()))\"   ✓\n        |      |      |→ \"((()\" → \"((())\" ... \n        |      |→ \"(()\" → \"(()(\" → \"(()()\" → \"(()())\"  ✓\n        |             |→ \"(())\" → \"(())(\" → \"(())()\"  ✓\n        |→ \"()\" → \"()(\" → \"()((\" → \"()(()\" → \"()(())\"  ✓\n                      |      |→ \"()()\" → \"()()(\" → \"()()()\"  ✓\n\n  5 valid combinations\n```",
